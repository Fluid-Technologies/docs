...
The GitHub App is **Active** on the org. **Free plan — no API keys.**

<Warning>
A `git push` to `docs` does **not** always trigger a build. Mintlify may only auto-deploy when an **editor or admin** merges via the web editor. Direct pushes (local or CI) often require a **manual Deploy** from the dashboard. See [deployment permissions](https://www.mintlify.com/docs/dashboard/permissions).
</Warning>

If Activity shows the last deploy was days ago while GitHub has newer commits, click **Deploy** / trigger a manual update ([manual deploy](https://www.mintlify.com/docs/deploy/deployments)).
...
| Push to docs did not deploy | **Expected** for non-editor pushes — click **Deploy** in dashboard; confirm you are Mintlify **admin/editor** |
| Activity stuck on old "Manual Update" | Trigger new **Deploy**; revalidate Git Settings (green checkmark); check GitHub **Webhooks** on `docs` repo |
| Preview unavailable | Usually follows a failed or stale build — redeploy and check Activity log for errors |
