...
Get authorized in a few minutes. This guide assumes you already have API credentials from the Connect dashboard (**API Keys**). It does not cover running `fluide-auth` locally — that is internal platform operations.
...
| Variable | Example |
| --- | --- |
| `FLUIDE_BASE_URL` | `https://staging.api.fluidehr.com` |
...
```javascript Node.js
const baseUrl = process.env.FLUIDE_BASE_URL ?? 'https://staging.api.fluidehr.com';
...
```bash curl
curl -X GET "$FLUIDE_BASE_URL/api/v1/developer-access/current" \
  -H "Authorization: Bearer $FLUIDE_ACCESS_TOKEN" \
  -H "X-Fluide-Client-Id: fluide-developer"
```

```javascript Node.js
const res = await fetch(`${baseUrl}/api/v1/developer-access/current`, {
  headers: {
...
<CardGroup cols={2}>
  ...
  <Card title="Auth overview" icon="book" href="/auth/overview">
    Credential lifecycle and how auth fits the gateway.
  </Card>
</CardGroup>
