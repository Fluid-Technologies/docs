...
Product APIs are proxied through the Tyk gateway. The playground prompts for three credentials on every product route (including health checks):
...
Click **Authorize** before **Send**. If you skip it, the gateway returns `400 Authorization field missing` or `401` with codes such as `MISSING_API_KEY` or `CLIENT_AUDIENCE_MISMATCH`.
