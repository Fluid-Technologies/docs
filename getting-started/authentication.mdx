...
Every product endpoint in [API reference](/api-reference) includes an interactive playground. The Tyk gateway rejects requests without the right headers — you will see `Authorization field missing` or `401` if any are omitted.
...
Click **Send**. Health checks such as `GET /api/v1/hr/health` use the same headers as business endpoints — there is no anonymous access through the gateway.
</Step>

</Steps>

<Tip>
Gateway middleware validates that `X-Fluide-Client-Id` matches the JWT `fluide_client_id` claim and that `X-Fluide-Api-Key` matches `dev_api_key` on machine tokens. See [Fluide Gateway hybrid RBAC contract](https://github.com/Fluid-Technologies/FluideGateway/blob/main/docs/hybrid-rbac-contract.md) for the full inbound identity contract.
</Tip>
