...
- Developer `apiKey` and `apiSecret` from the Connect dashboard — see [Authentication](/getting-started/authentication)
...
Open any product endpoint in [API reference](/api-reference), click **Authorize**, and paste your `accessToken`, `fl_dev_...` API key, and `fluide-developer` client ID. See [Authentication](/getting-started/authentication) for the full playground walkthrough.
